...
```{r Ex_f}
visitors %>% 
  filter(50000 <= n_2022 & n_2022 <= 100000) %>% count()
```
...
visitors %>% 
  group_by(region) %>%
  summarise(num_attr = n_distinct(attraction)) %>%
  filter(num_attr > 50)
```
...
#Some data is unavailable therefore it cannot be computed in either of the calculations
